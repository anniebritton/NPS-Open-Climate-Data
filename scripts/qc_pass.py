...
def main() -> None:
    ...
    lines.append(
        "1. **Unit / range** — every variable's annual values fall inside a hard "
        "plausibility band (e.g. mean temperature in [−25, 35] °C, snow cover in "
        "[0, 100] %). A FAIL means a variable somewhere in the pipeline went "
        "outside what's physically reasonable for any US National Park.\n"
        "2. **Annual aggregation arithmetic** — for the QC sample of "
        f"{len(EXTERNAL_BOUNDS_C)} parks, recomputes `mean(tmean_c)` and "
        "`sum(prcp_mm)` per year from the raw daily CSV and compares to the "
        "pipeline's `annual` block. Catches regressions in the SUM_VARS / "
        "MEAN_VARS dispatch in `analysis.annual()`.\n"
        "3. **Trend-test consistency** — `significant_95 == (p_value < 0.05)` "
        "for every per-variable trend on every park. Catches drift between the "
        "Mann–Kendall p-value and the cached significance flag.\n"
        "4. **External cross-check** — for the QC sample, compares the headline "
        "`tmean_c` total warming over 1980–present against per-park bounds "
        "derived from Gonzalez et al. (2018) and NOAA Climate at a Glance "
        "state-level series. Bounds are deliberately wide (covering 30+ years "
        "of inter-annual noise around the published trend) — failures here "
        "would indicate a real pipeline regression, not a debate about the "
        "true rate.\n"
    )
